        ...
        <Card className="!p-0 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {purchases.map((p) => (
                  <tr key={p.id} className="border-b border-gold/[0.06] last:border-0 hover:bg-gold/[0.03]">
                    <td className="px-4 py-3 text-ash whitespace-nowrap">{new Date(p.created_at).toLocaleDateString()}</td>
                    <td className="px-4 py-3 text-ivory font-medium whitespace-nowrap">{p.amount_paid.toLocaleString()}</td>
                    <td className="px-4 py-3 text-ash">{p.payment_currency}</td>
